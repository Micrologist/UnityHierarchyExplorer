...
namespace MemUtil
{
        ...
        IEnumerable<IntPtr> ScanInternal(SigScanTarget target, int align)
        {
            ...
            foreach (SigScanTarget.Signature sig in target.Signatures)
            {
                ...
                foreach (int off in new ScanEnumerator(_memory, align, sig))
                {
                    ...
                    {
                        string[] sigBytes = sig.Pattern.Select((b, i) =>
                        {
                            if (sig.Mask != null && sig.Mask[i]) return "??";
                            return b.ToString("X2");
                        }).ToArray();
                        ptr = target.OnFound(_process, this, ptr, sig.Offset, String.Join(" ", sigBytes));
                    }
                    yield return ptr;
                }
            }
    ...
    public class SigScanTarget
    {
        ...
        public delegate IntPtr OnFoundCallback(Process proc, SignatureScanner scanner, IntPtr ptr, int offset, string signature);
